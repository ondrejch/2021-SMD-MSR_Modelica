...
%% Transient - 100pmc Step Insertion
%%% Runs a simulation of 100pcm step insertion
...
simtime = 3000;                                                            %Simulation time [s]
...
reactdata = [0 0 100E-5 100E-5];                                           %Reactivity insertions [abs]
reacttime = [0 2000 2000 3000];                                            %Reactivity insertion time [s]
...
%%% Pump Trips
Trip_P_pump=2000000;                                                       %Time at which primary pump is tripped [s]
Trip_S_pump=2000000;                                                       %Time at which secondary pump is tripped [s]
...
UHX_MODE = 2;
Block_UHE=2000000;                                                         %Time at which ultimate heat exchanger will be cut off [s]
...
DHRS_MODE = 1; 
DHRS_time = 2000000;                                                       %Time at which DRACS will be activated. use simtime to keep it off [s]

%%% Only for DHRS_MODE = 1
DHRS_Power = P*(0.040);                                                    %Maximum power that can be removed by DHRS
Power_Bleed = P*(0.000);                                                   %Some power will removed from DRACS even when its not used 
...
save('tran1R1Slink.mat','tout','powN','powF','powD','tIn','tGrap','tOut','rho_fb_tot','rho_fb_f','rho_fb_g')
