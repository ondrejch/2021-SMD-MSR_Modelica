package Transient1 model MSREoneR
  SMD_MSR_Modelica.Nuclear.mPKE mPKE(Lam = 2.400E-04, beta = {0.000223, 0.001457, 0.001307, 0.002628, 0.000766, 0.00023}, lam = {1.240E-02, 3.05E-02, 1.11E-01, 3.01E-01, 1.140E+00, 3.014E+00}, omega = 10, rho_0 = 0.002465140767843, sinInsertionTime = 20000000, sin_mag = 0, stepInsertionTime = 2000, step_mag = 100E-5, tauCoreNom = 8.46, tauLoopNom = 16.73) annotation(
    ...
    SMD_MSR_Modelica.HeatTransport.SecondaryPump secondaryPump(freeConvectionFF = 0.05, secondaryPumpK = 0.2, tripSecondaryPump = 200000) annotation(
      Placement(visible = true, transformation(origin = {1710, -1240}, extent = {{-362, -362}, {362, 362}}, rotation = 0)));
    ...
    SMD_MSR_Modelica.HeatTransport.UHX uhx(m_PN1UHX = 3.924401289158446e+02, UHXP = 8, cP_pFluidUHX = 2.39E-3, m_dot_pFluidUHXnom = 103.0701, tauUHXtoPHX = 8.24, tripUHX = 20000000, SetDemand = 8) annotation(
  ...
  equation
    ...
    connect(secondaryPump.secondaryFlowFrac, PHX.secondaryFF) annotation(
      Line(points = {{1710, -1457}, {1626, -1457}, {1626, -54}, {2232, -54}, {2232, 116}}, color = {245, 121, 0}));
    ...
    connect(secondaryPump.secondaryFlowFrac, uhx.secondaryFlowFrac) annotation(
      Line(points = {{1710, -1457}, {1836, -1457}, {1836, -704}}, color = {245, 121, 0}));
  ...
  equation
    ...
    connect(nineRcore.decayHeat_Out, PHX.P_decay) annotation(
      Line(points = {{-1614, 76}, {-1582, 76}, {-1582, 1104}, {1810, 1104}, {1810, 894}}, color = {0, 225, 255}));
  connect(dhrs.DHRS_TempOUT, PHX.T_in_pFluid) annotation(
      Line(points = {{40, 714}, {1256, 714}, {1256, 810}}));
  connect(nineRcore.decayHeat_Out, dhrs.DHRS_DecayHeat) annotation(
      Line(points = {{-1614, 76}, {-1594, 76}, {-1594, 520}, {-488, 520}}, color = {0, 225, 255}));
  connect(nineRcore.temp_Out, dhrs.DHRS_tempIN) annotation(
      Line(points = {{-1096, 76}, {-1026, 76}, {-1026, 714}, {-488, 714}}));
  connect(primaryPump.primaryFlowFrac, dhrs.fuelFlowFrac) annotation(
  ...
  connect(nineRcore.decayHeat_Out, pipe.PiDecay_Heat) annotation(
      Line(points = {{-1614, 76}, {-1414, 76}, {-1414, 142}, {168, 142}, {168, -575}, {167, -575}}, color = {0, 225, 255}));
  connect(PHX.T_out_pFluid, pipe.PiTemp_IN) annotation(
